		...
		const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		printk(KERN_INFO "i2c_check_functionality error 2\n");
		goto exit;
	}
	...
	if (bus) {
		pr_debug("add symlink 2-0018 to device");
		err = sysfs_create_link(&bus->p->devices_kset->kobj,
						&client->dev.kobj, "2-0018");
	}
...
static int bma250_remove(struct i2c_client *client)
{
	...
	if (bus) {
		pr_debug("remove symlink 2-0018");
		sysfs_remove_link(&bus->p->devices_kset->kobj,"2-0018");
	}
